...
public class D_Transe {
		...
		for (int i = 0; i < zero_L.size(); i++) {
			for (int j = 0; j < array_inputString.size(); j++) {
				if (array_inputString.get(j).contains("L")) {
					...
					String str = array_inputString.get(j).substring(0, index).trim();
					if (str.equals(zero_L.get(i))) {
						if(!zero_L.contains(array_inputString.get(j).substring(array_inputString.get(j).indexOf("L") + 1,
	...
	public List<String> compare() {
		...
		//System.out.println(words[c]+"="+zero_L.toString());
		//c++;
		...
		for(int l = 0 ; l < c ; l++) {
			dfa.add(words[0]+firstOut.get(o)+words[l]);
			o++;
		}
		 *///System.out.println("--------------------------");
		List<String> list=new ArrayList<String>();
		list.add(zero_L.toString());
		...
		for(int j = 0 ; j<list.size();j++) {
			...
			if(list.get(j).length()>3) {
				...
				for(int k = 0 ; k < values.size();k++) {
					if(existance(list, values.get(k))) {
						for(int n = 0; n<list.size();n++) {
							if(compareTowList(list.get(n), values.get(k))) {
	...
	public boolean compareTowList(String first,String second) {
		int counter = 0 ;
		String[] s0 = toString(first).split(",");
		String[] s1 = toString(second).split(",");
	...
	public boolean existance(List<String> firstIp,String secondIp) {
		for(int i = 0 ; i < firstIp.size() ; i++) {
			if(compareTowList(firstIp.get(i),secondIp)) {
				return true;
			}
		}
		return false;
